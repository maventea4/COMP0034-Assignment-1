...
def load_crime_data():
    ...
    try:
        ...
        crime_df.rename(columns={'MajorText': 'MajorCrimeCategory', 'MinorText': 'CrimeSubcategory'}, inplace=True)
        return crime_df
...
app.layout = html.Div([
    dbc.Container([
        ...
        html.Div([
            ...
            html.P("*The graph is interactive. You can zoom in on specific dates using the lasso tool or crop feature.",
        ...
        html.Div([
            html.P("Data sourced from the Metropolitan Police Service under the UK Open Government License (OGL v2).", style={"color": "#b0c4de", "text-align": "center", "margin-top": "10px"})
        ], className="mt-5")
...
def update_graphs_and_dropdown(n_clicks, selected_major_crime, selected_borough):
    ...
    pie_chart_fig = px.pie(
        pie_chart_df, names='MajorCrimeCategory', values='CrimeCount',
    ...
    if selected_major_crime:
        breakdown_df = filtered_df[filtered_df['MajorCrimeCategory'] == selected_major_crime]
        breakdown_fig = px.line(
            breakdown_df, x='Month', y='CrimeCount', color='CrimeSubcategory',
    ...
    major_crime_options = [{'label': mc, 'value': mc} for mc in filtered_df['MajorCrimeCategory'].unique()]
